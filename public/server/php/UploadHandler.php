...
$host="srv-db-plesk05.ps.kz";
$db="eclinick_inventory";
$user="eclin_inventory";
$pass="vK8bb1!6";
$connect_sql=mysql_connect($host,$user,$pass);
mysql_query("SET NAMES `utf8`", $connect_sql);
@mysql_query("use $db", $connect_sql);
...

{
    ...
    protected $error_messages = array(
        ...
        1 => 'Файл превышает размер в настройках php.ini',

        2 => 'Файл превышает размер 5Мб',

        3 => 'Файл был загружен только частично',

        4 => 'Файл не был загружен',

        6 => 'Временная папка не найдена',

        7 => 'Не удалось записать файл на диск',

        8 => 'Расширение PHP остановил загрузку файлов',

        'post_max_size' => 'Файл превышает размер в настройках php.ini',

        'max_file_size' => 'Файл слишком большой',

        'min_file_size' => 'Файл слишком маленький',

        'accept_file_types' => 'Неверный тип файла',

        'max_number_of_files' => 'Превышен лимит одновременной закачки файлов',

        'max_width' => 'Картинка большого размера',

        'min_width' => 'Картинка маленького размера',

        'max_height' => 'Картинка большого размера',

        'min_height' => 'Картинка маленького размера'
    ...
    function __construct($options = null, $initialize = true, $error_messages = null) {
        ...
        $this->options = array(
            ...
            'upload_dir' => $_SERVER['DOCUMENT_ROOT'].'/upload/images/',

            'upload_url' => $_SERVER['DOCUMENT_ROOT'].'/upload/images/',
    ...
    protected function get_file_objects($iteration_method = 'get_file_object') {
		...
		$arr_img = array();
		$file_new = array();
		$s = mysql_query("select * from catalog where id=$id");
		$r = mysql_fetch_array($s);
		
		$arr_img = explode(';',$_GET["files"]);
		
		if ($arr_img[0]=='') return 0;
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error,
        ...
        $file->name = rand(0,100000)."_".rand(500,1000000)."_".date("H").'.'.end(explode('.',$name));
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
           ...
           /* $s = mysql_query("select * from i_block where id=".intval($_GET["id"])."");
			...
			$images = explode(';',$r["doc1_block"]);
			
			if (sizeof($images)<5)
				$images[] = $file->name;
			
			mysql_query("update i_block set doc1_block='".join(';', $images)."' where id=".intval($_GET["id"])."");
          	*/
            $this->set_file_delete_properties($file);
            ...
            foreach($this->options['image_versions'] as $version => $options) {
                ...
                if (!empty($version)) {
                    ...
                    if (is_file($file)) {
						...
						mysql_query("update i_block set doc1_block='".join(';', $new_images)."' where id=".intval($_GET["id"])."");
						*/
