...
@section('content')
    ...
    <ul class="nav nav-tabs">
        ...
        <li ><a href="{{route('admin.peremesh.list_oper')}}">Переместить оборудование</a></li>

        <li class="active"><a href="{{route('admin.peremesh.list_pere')}}">История перемещение</a></li>
    ...
    <script>
        ...
        function get_ser_page(num) {
            ...
            $.ajax({
                url: "{{ route('admin.peremesh.list_pere_search')}}",
                method: 'POST',
    ...
    <form name="form" method="post" action="{{route('admin.peremesh.list_pere_search')}}" rel="ajax_form">
        @csrf
    ...
    <table class="table-striped table-hover table-border" width="100%">
        ...
        <tr>
            ...
            <th align="center">Пользователь</th></tr>
        </thead>
        <tbody>
        @foreach($pere as $c)
